...
import numpy as np
# probs is an array of probability predicted by each classifier for an instance (x,y)

def weighted_vote_rule(probs):
    ...
    return (np.mean(probs)>0.5).astype(int)
...
def majority_vote_rule(probs):
    ...
    return np.bincount(probs>0.5).argmax()
